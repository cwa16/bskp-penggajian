...
class WhatsAppController extends Controller
{
    ...
    public function store(Request $request)
    {
        if ($request->hasFile('pdf')) {
            ...
            $path = $file->storeAs('public/pdf', $filename . '.pdf');
    ...
    public function storeFile(Request $request)
    {
    	...
    	$pdfPaths = $request->file('pdf');
	$filename = $request->input('filename');
	$pdfPath = $pdfPaths->storeAs('public/pdf', $filename . '.pdf');

    	return response()->json(['path' => $pdfPath], 200);
    }

    public function storeFileHariRaya(Request $request)
    {
    	// Store the file on the server
    	$pdfPaths = $request->file('pdf');
	$filename = $request->input('filename');
	$pdfPath = $pdfPaths->storeAs('public/pdf', $filename . '.mp4');
